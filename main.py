...
for epoch in range(num_epochs):
    ...
    for inputs, labels in trainloader:
            ...
            # backward
            loss.backward() # the grads are from now on no longer none
            ...
            ist:
            """
            _outputs_max_idx = torch.argmax(inputs, dim=1) # indexthat contains maximal value per row (prediction per sample in batch)
            
            _outputs = torch.gather(inputs, dim=1, index= _outputs_max_idx.unsqueeze(1)) # gather sammelt outputs aus y entlang der Axe 
